    ...
    if (isset($_POST['first']) && isset($_POST['last']) && isset($_POST['email']) && isset($_POST['user']) && isset($_POST['pass']) && isset($_POST['pass-confirm'])) {
        ...
        if (empty($first_name)) {
            ...
            if (exist_email($email)) {
                $error = "This email is exist in other account";
            } else if (exist_username($user)) {
                $error = "This usename is exist in other account";
            } else {
                $result = register($user, $pass, $first_name, $last_name, $email);
                if ($result['code'] != 0) {
                    $error = $result['message'];
                } else {
                    $message_success = "Create account successful, please check your email to active your account";
                    $error = "";
                }
            }
        }
